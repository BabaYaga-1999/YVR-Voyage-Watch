...
while True:
    ...
    if len(result.boxes) > 0:
        for box in result.boxes.data:
            ...
            if label == 'person' and score >= 0.5:
                ...
                # Basic fall detection based on aspect ratio
                # If aspect ratio is less than a certain threshold, it might indicate a fall
                if aspect_ratio < 0.5:  # This threshold is arbitrary and may need adjustment
                    fall_detected = True
    ...
    # Indicate fall detection on the frame
    if fall_detected:
        cv2.putText(frame, 'FALL DETECTED!', (5, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
